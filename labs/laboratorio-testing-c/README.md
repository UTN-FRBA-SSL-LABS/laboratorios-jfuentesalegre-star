...
> R:le da un el valor 0 a la variable cantidad. Es importante porque nos aseguramos que su valor inicial sea 0 y no otro.
...
> R:Linea 24. Hace la suma del total de productos unitarios.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Cada precio de un producto deberia multiplicarse por su cantidad.
...
> R:La falla era c->cantidad <= MAX_ITEMS y solo tendria que tener el < .
...
> R:Un test unitario es un test que se le hace a una unica funcion. Mentras que uno de integracion verifica varias funciones combinadas. El que detecto primero el bug fue el "test unitario"
...
> R:Es peligroso en C pues te da mas libertad o mas control. Mientras que Java o Python tiene control sobre el espacio reservado.
...
> R:Lo mejor es que te permite identinficar errores que pasaron desapercibidas en el codigo.
...
> R:Porque solo verifica el precio de un producto. Mientras que en el test_total_con_cantidad se evalua el precio * cantidad.
